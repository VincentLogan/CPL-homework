...
int main(){
    ...
    printf("第七题【判断题】:敬礼时，手腕伸直，五指并拢，中指微接太阳穴\nA. 正确\nB. 错误\n请输入你的答案:");
    scanf("%c",&answer[7]);
    if(answer[7]=='B')
    {
        sum+=10;
    ...
    if(sum<60)
    {
        printf("你的总分为：%d【不及格】\n\n​“始于条令，归于条令”​。请你认真复习错题，从零开始，认真背诵和理解条令，迎头赶上！我们期待你下一次的显著进步！\n",sum);
    }
    else if(sum>=60&&sum<=80)
    {
        printf("你的总分为：%d【良好】\n“平时多流汗，战时少流血”​。理论是实践的先导，本次成绩表明你已为训练打下了良好基础，但距离“顶尖”还差最后一公里。不要满足于“差不多”，要追求“百分百”，望你刻苦钻研，力争满分!\n",sum);
    }
    else
    {
        printf("你的总分为：%d【优秀】\n“武艺练不精，不算合格兵”​，你用实际行动证明了这一点。本次问答成绩突出，证明你在理论层面已是一名“精兵”。接下来，要在训练实践中将理论发挥到极致，争取成为队列训练的“标兵”!\n",sum);
    }
    return 0;
}